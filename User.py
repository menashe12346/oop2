        ...
        if other_user.user_name in User.users:  # Checking if the other user exists
            if self.logged_in:  # Checking if the current user is logged in
                if other_user not in self.followers:  # Checking if the current user is not already following the other user
                    ...
                    other_user.users_who_follow_me.append(self)  # Adding the current user to the other user's followers list
                    print(self.user_name + " started following " + other_user.user_name)  # Printing a message indicating the follow action
        ...
        if other_user.user_name in User.users:  # Checking if the other user exists
            if self.logged_in:  # Checking if the current user is logged in
                if other_user in self.followers:  # Checking if the current user is following the other user
                    ...
                    other_user.users_who_follow_me.remove(self)  # Removing the current user from the other user's followers list
                    print(self.user_name + " unfollowed " + other_user.user_name)  # Printing a message indicating the unfollow action
